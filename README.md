...
- **`@stamp-card/shared`**: Common TypeScript types and interfaces shared between backend and frontend
- **`@stamp-card/backend`**: NestJS backend server
- **`@stamp-card/frontend`**: React frontend application

The shared package is compiled from TypeScript to JavaScript, allowing both backend and frontend to import the same type definitions. The `dist/` folder is auto-generated and excluded from version control.
...
This command will:
1. Build the shared package (`@stamp-card/shared`)
2. Start the NestJS backend in watch mode
...
This command will:
1. Build the shared package (if not already built)
2. Start the Vite dev server

The frontend will be available at http://localhost:5173

**Note:** Both commands automatically build the shared package because both backend and frontend depend on the compiled TypeScript types from `packages/shared/dist/`.
...
The shared package contains TypeScript types used by both backend and frontend. When you modify files in `packages/shared/src/`:

### During Development

The `npm run backend` and `npm run frontend` commands automatically rebuild the shared package before starting. However, if you're already running them and make changes to shared types:

```bash
# Rebuild shared package manually
npm run build:shared

# Both backend and frontend will hot-reload with the new types
```

### Common Scenarios

**Adding a new type:**
1. Create/edit files in `packages/shared/src/types/`
2. Export it from `packages/shared/src/index.ts`
3. Run `npm run build:shared` (or restart backend/frontend)
4. Import in backend/frontend: `import { YourType } from '@stamp-card/shared'`

**TypeScript errors about missing types:**
```bash
# Clean and rebuild the shared package
...
- Contains compiled JavaScript and type definition files
